...
/**
 * Used to always display host part in URLs for multi-language websites without modifying JRoute::_() function
 */
class JoomSefDomainUri extends JUri
{
    /**
     * Custom property for host, because Route::_() overwrites host for SSL URLs
     *
     * @var string
     */
    protected $joomsefHost = null;

    /**
     * Stores current host in JoomSEF host property
     */
    public function storeHost()
    {
        $this->joomsefHost = $this->getHost();
    }

    /**
     * If host is set and is different from current host, always displays it
     *
     * @param array $parts
     * @return string
     */
    ...
    {
        // Replace host with JoomSEF host if set
        if ($this->joomsefHost)
            $this->setHost($this->joomsefHost);

        // If host is not in $parts, check if host is set and is different from current host
        if (!in_array('host', $parts) && $this->getHost()) {
            $host = $this->getHost();
            ...
            if ($host != $curHost) {
                // Output scheme and host
                if (!in_array('scheme', $parts)) {
                    $parts[] = 'scheme';
                }
                $parts[] = 'host';

                // Generate scheme if not set, just to be sure
                if (!$this->getScheme()) {
                    $this->setScheme($this->isSSL() ? 'https' : 'http');
                }
            }
        }

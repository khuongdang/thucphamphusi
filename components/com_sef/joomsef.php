...
class JoomSEF
{
                        ...
                        if (JoomSEF::_isHomePage($uri2, true)) {
                            ...
                            if ($sefConfig->langEnable && ($sefConfig->langPlacementJoomla == _COM_SEF_LANG_DOMAIN)) {
                                if (!empty($lng) && isset($sefConfig->subDomainsJoomla[$lng])) {
                                    $uri = SEFTools::createDomainUri($uri, $sefConfig->subDomainsJoomla[$lng]);
                                }
                            }
                        ...
                        if (!$sefUrl || $check) {
                            ...
                            if (is_object($data) && is_a($data, 'JURI')) {
                                ...
                                if(strlen($subdomain)) {
                                    ...
                                    $uri = SEFTools::createDomainUri($uri, $subdomain.'.'.$curHost);
                                }
    ...
    static function getLanguage($uri) {
        ...
        if (strlen($lang)) {
            ...
            if (JFactory::getLanguage()->getTag() != $lang_code) {
                ...
                // #23150: Load library language files
                if (!$jLang->load('lib_joomla', JPATH_SITE, null, false, true)) {
                    $jLang->load('lib_joomla', JPATH_ADMINISTRATOR, null, false, true);
                }
            }
    ...
    static function _titleToLocation(&$title)
    {
        ...
        if( $sefConfig->lowerCase ) {
            $title = Util::stringURLSafe(JoomSEF::_toLowerCase($title));
        }
    ...
    static function _storeLocation(&$data, $check = false, $removeItemid = false)
    {
        ...
        // #23450: Causes problems with Joomla native multilanguage due to different Itemids
    ...
    static function _checkRow(&$row, $ignoreSource, $Itemid, $url, &$metadata, $temploc, $priority, $option,$check=false,$host, $sitemapParams = null)
    {
        ...
        if( ($row != false) && !is_null($row) ) {
            ...
            // #23450: Don't number homepage links if domains are used for languages
            // and URLs differ only in lang variable (ignoring Itemid)
            $dontNumber = false;
            if ($temploc == '' && $sefConfig->langPlacementJoomla == _COM_SEF_LANG_DOMAIN) {
                if (SEFTools::removeVariable($row->origurl, 'lang') == SEFTools::removeVariable($url, 'lang')) {
                    $dontNumber = true;
                }
            }

            if (!$numberDuplicates || $dontNumber) {
    ...
    static function _saveNewURL($Itemid, &$metadata, $priority, $temploc, $url,$host, $sitemapParams)
    {
        ...
        $data['sefurl'] = $temploc;
        $data['origurl'] = html_entity_decode(urldecode($url));
        $data['Itemid'] = empty($Itemid) ? null : $Itemid;

        if (is_array($metadata) && count($metadata) > 0) {
            foreach($metadata as $metakey => $metaval) {
                $data[$metakey] = $metaval;
            }
        }
        ...
        $data['trace'] = null;
        $sefConfig = SEFConfig::getConfig();
        if (!empty($sefConfig->trace)) {
            $data['trace'] = JoomSEF::_getDebugInfo($sefConfig->traceLevel);
        }

        $data['locked'] = (int)$sefConfig->autolock_urls;

        // Sitemap default values
        $data['sm_indexed'] = (isset($sitemapParams['indexed']) ? $sitemapParams['indexed'] : ($sefConfig->sitemap_indexed ? 1 : 0));
        $data['sm_date'] = date('Y-m-d');
        $data['sm_frequency'] = (isset($sitemapParams['frequency']) ? $sitemapParams['frequency'] : $sefConfig->sitemap_frequency);
        $data['sm_priority'] = (isset($sitemapParams['priority']) ? $sitemapParams['priority'] : $sefConfig->sitemap_priority);

        $data['host'] = $host;

        // #23445: Prevent duplicate real URLs without unique key (because Itemid can be NULL)
        // Prepare lists
        $cols = array_map(array($db, 'quoteName'), array_keys($data));
        $vals = array();
        foreach ($data as $key => $val) {
            $vals[] = (is_null($val) ? 'NULL' : $db->quote($val))." AS ".$db->quoteName($key);
        }
        $where = array();
        $where[] = "`origurl` = ".$db->quote($data['origurl']);
        $where[] = "`Itemid` ".(is_null($data['Itemid']) ? "IS NULL" : "= ".$db->quote($data['Itemid']));
        ...
        $query .= ") SELECT ";
        $query .= implode(', ', $vals);
        $query .= " FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `#__sefurls` WHERE ";
        $query .= implode(' AND ', $where);
        $query .= ")";

        $db->setQuery($query);

        // #25634: Don't display any error
        try {
            if (!$db->query()) {

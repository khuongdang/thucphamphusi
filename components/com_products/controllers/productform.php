 ...
 * @since  1.6
 */
class ProductsControllerProductForm extends JControllerForm
{
	/**
	...
	public function edit($key = NULL, $urlVar = NULL)
	{
		...
		// Get the model.
		$model = $this->getModel('ProductForm', 'ProductsModel');
	...
	public function save($key = NULL, $urlVar = NULL)
	{
		...
		if ($data === false)
		{
			...
			$app->setUserState('com_products.edit.product.data', $jform);

			// Redirect back to the edit screen.
			$id = (int) $app->getUserState('com_products.edit.product.id');
			$this->setRedirect(JRoute::_('index.php?option=com_products&view=productform&layout=edit&id=' . $id, false));
		...
		$item = $menu->getActive();
		$url  = (empty($item->link) ? 'index.php?option=com_products&view=products' : $item->link);
	...
	public function cancel($key = NULL)
	{
		...
		$editId = (int) $app->getUserState('com_products.edit.product.id');
